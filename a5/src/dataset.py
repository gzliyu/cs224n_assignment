...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Randomly truncate the document to a length no less than 4 characters,
        # and no more than int(self.block_size*7/8) characters.
        truncate_len = random.randint( 4, int(self.block_size * 7/8) ) # 截断之后的长度
        truncate_len = min(entry_len, truncate_len)
        truncated_entry = entry[:truncate_len]
        ...
        assert truncate_len >= 4, (entry_len, truncate_len, masked_content_len, entry, idx)
        ...
        prefix = truncated_entry[:prefix_len]
        masked_content = truncated_entry[prefix_len:prefix_len + masked_content_len]
        suffix = truncated_entry[prefix_len + masked_content_len:]
