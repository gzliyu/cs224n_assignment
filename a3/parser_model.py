...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        ###        Initialize weight with the `nn.init.xavier_uniform_` function and bias with `nn.init.uniform_`
        ###        with default parameters.
        ### FIXME: 看看权重矩阵的大小是不是(embed_size, hidden_size)
        ### NOTE: bias需要用`nn.init.uniform_`初始化，`nn.init.xavier_uniform_`必须至少是二维的
        ###       Trainable variables are declared as `nn.Parameter` which is a commonly used API
        ...
        ###             http://andyljones.tumblr.com/post/10998971763/an-explanation-of-xavier-initialization
        ###
        ...
        # 词向量嵌入->隐藏层
        W = nn.Parameter( torch.empty((self.n_features * self.embed_size, self.hidden_size)) ) # 嵌入矩阵到隐藏层输入中的 权重矩阵 dm * hidden_size
        b1 = nn.Parameter( torch.empty( hidden_size ) )
        # nn.init.xavier_uniform_(W)  初始化权重矩阵W
        ...
        self.hidden_logits_weight = nn.Parameter(nn.init.xavier_uniform_(U))
        self.hidden_logits_bias = nn.Parameter(nn.init.uniform_(b2))
        # 构建Dropout层
    ...
    def embedding_lookup(self, w):
        ...
        ###       用词向量表示词在一般情况下比较耗费内存，所以模型的输入是词索引的列表，词索引为一个整数。这个函数要实现 "词索引 -> 词向量"
        ###       如果用循环会返回一个全零向量（idk why）， 另外循环效率也低
        ###       题目的意思是用index_select在embedding中选出目的词向量，用gather调整形状
        ...
        x = torch.index_select(self.embeddings, 0, torch.flatten(w))    # (batch_size * n_features, embed_size)
        x =  x.view((w.shape[0], self.n_features * self.embed_size))    # 输出格式：(batch_size, n_features * embed_size)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        Note:  这里是token，需要用embedding_lookup转换一下 (batch_size, n_features) -> (batch_size, n_features * embed_size)
        ...
        logits = torch.matmul(h, self.hidden_logits_weight) + self.hidden_logits_bias
        ### END YOUR CODE
        return logits
