...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###                 last element of the list.   等待处理的词
        ###     self.buffer: The current buffer represented as a list with the first item on the
        ###                  buffer as the first item of the list   还没处理的词
        ###     self.dependencies: The list of dependencies produced so far. Represented as a list of
        ###             tuples where each tuple is of the form (head, dependent).
        ###             Order for this list doesn't matter. 已经处理好的依赖关系
        ###
        ### Note: The root token should be represented with the string "ROOT"
        ### Note: If you need to use the sentence object to initialize anything, make sure to not directly 
        ###       reference the sentence object.  That is, remember to NOT modify the sentence object. 
        self.stack = ["ROOT"]   # 初始状态下只有根节点
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            raise Exception("parse step出现预期外的命令")
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    有很多个sentense，输出很多个dependency；传进来一个model，model.predict([PartialParse的list])输出transition的决策（s、la、ra）
    ...
    partial_parses = [ PartialParse(sentence) for sentence in sentences ]   # Initialize partial parses as a list of PartialParses, one for each sentence in sentences
    unfinished_parses = partial_parses.copy()   # Initialize unfinished parses as a shallow copy of partial parses
    while len(unfinished_parses):   # 当不为空时
        minibatch = unfinished_parses[:batch_size].copy()  # Take the first batch size parses in unfinished parses as a minibatch
        transitions = model.predict(minibatch)  # Use the model to predict the next transition for each partial parse in the minibatch
        for k, pp in enumerate(minibatch):
            pp.parse_step(transitions[k])
