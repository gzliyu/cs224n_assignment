...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # Note combined_output_project in and out
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        @returns scores (Tensor): a v`ariable/tensor of shape (b, ) representing the
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Note: source是一个nn.Embedding类，embedding（index_list）返回对应的嵌入向量
        X = self.model_embeddings.source(source_padded)
        X = pack(X, lengths=torch.tensor(source_lengths))
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = unpack(enc_hiddens, batch_first=True)
        # Note: 拼接之后的向量是((b, h), (b, h))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Note: Y的形状是(tgt_len, b, e)，那么在每个循环,Y_t的形状是(1, b, e)
        for Y_t in torch.split(Y, split_size_or_sections=1, dim=0):
            Y_t = torch.squeeze(Y_t)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)
            next_dec_state, o_t, e_t = self.step(Ybar_t, dec_state,
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), mat2=enc_hiddens).squeeze()
    ...
    def generate_sent_masks(self, enc_hiddens: torch.Tensor, source_lengths: List[int]) -> torch.Tensor:
        ...
        """
        # print(enc_hiddens.batch_sizes[0], enc_hiddens.batch_sizes[1])
        enc_masks = torch.zeros(enc_hiddens.size(0), enc_hiddens.size(1), dtype=torch.float)
        # enc_masks = torch.zeros(enc_hiddens.batch_sizes[0], enc_hiddens.batch_sizes[1], dtype=torch.float)
